...
internal class PlayerFactoryTest {
    ...
    internal fun testCreateForSpectator_DeserializesObject() {
        ...
        with(player.getValue("team2")[2] as SpectatedPlayerImpl) {
            ...
            assertEquals(0, waterRunesActivated)
            assertEquals(2, bountyRunesActivated)
            ...
            assertEquals(9165, damageReceivedPreReductionPhysical)
            assertEquals(8381, damageReceivedPreReductionMagical)
            assertEquals(1942, damageReceivedPreReductionPure)
            assertEquals(5247, damageReceivedPostReductionPhysical)
            assertEquals(5730, damageReceivedPostReductionMagical)
            assertEquals(1443, damageReceivedPostReductionPure)
            assertEquals(2317, damageOutgoingPreReductionPhysical)
            assertEquals(12826, damageOutgoingPreReductionMagical)
            assertEquals(0, damageOutgoingPreReductionPure)
            assertEquals(1842, damageOutgoingPostReductionPhysical)
            assertEquals(6857, damageOutgoingPostReductionMagical)
            assertEquals(0, damageOutgoingPostReductionPure)
        }
    }
